(() => {
  ...
  const materialData = [
    ["Hormigón", "Masa / inercia / textura", "Lectura mineral y capacidad estructural, definida según proyecto, exposición y sistema de unión.", "assets/v10/material-hormigon.webp", "Pieza modular Nanca de hormigón arquitectónico"],
    ["Acero", "Precisión / luz / desmontaje", "Sección controlada y uniones mecánicas para piezas que necesitan exactitud, esbeltez y trazabilidad.", "assets/v10/material-acero.webp", "Pieza modular Nanca de acero cepillado"],
    ["Madera", "Calidez / ligereza / tacto", "Materia renovable cuando su origen y especificación se verifican; útil en estructura, envolvente e interior.", "assets/v10/material-madera.webp", "Pieza modular Nanca de madera laminada"],
    ["Plástico reciclado", "Recuperación / color / resistencia", "Compuesto a evaluar para elementos compatibles con su desempeño, mantenimiento y siguiente ciclo.", "assets/v10/material-reciclado.webp", "Pieza modular Nanca de plástico reciclado prensado"],
  ];
  $$(".material-drawer nav button").forEach((button, index) => button.addEventListener("click", () => {
    ...
    const image = $(".material-drawer figure > img");
    image.style.animation = "none";
    void image.offsetWidth;
    image.src = material[3];
    image.alt = material[4];
    image.style.animation = "";
    $(".material-drawer figure figcaption").textContent = `Material 0${index + 1} / ${material[0]}`;
  }));

  const states = { place: "paisaje", expression: "calma" };
  const recommendation = () => states.expression === "libre" ? "My Way" : states.expression === "materia" ? "Line" : states.place === "urbano" ? "Basic" : "Natura";
  const statements = { Basic: "Lo esencial, muy bien medido.", Line: "Una línea que ordena el paisaje.", Natura: "Habitar sin separarse del lugar.", "My Way": "El sistema, a tu manera." };
